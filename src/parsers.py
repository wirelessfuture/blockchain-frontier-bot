def market_data_parser(market_data: dict, coin: str) -> list:
    ...
    current_prices = [
        f"{key.upper()}: {value}\n"
        for key, value in market_data["current_price"].items()
    ...
    price_change_percentage_24h = [
        f"{key.upper()}: {value}%\n"
        for key, value in market_data["price_change_percentage_24h_in_currency"].items()
        if key in currency_keys
    ]
    ...
    price_change_24h = [
        f"{key.upper()}: {value}\n"
        for key, value in market_data["price_change_24h_in_currency"].items()
        if key in currency_keys
    ]
...
def eth_gas_price_parser(gas_data: dict) -> list:
    ...
    gas_prices = [
        f"{key}: {int(value/10)} GWEI\n"
        for key, value in gas_data.items()
        if key in price_keys
    ]
    gas_prices.insert(0, "⛽️ ETH Gas Prices ⛽️\n")
    gas_prices.append("\n")
